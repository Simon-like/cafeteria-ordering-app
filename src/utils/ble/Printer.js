...
class Printer {
  ...
  print(text, nowrap = false) {
    let encoded = Array.from(gbk_build.encode(text))
  ...
  printLR(text1, text2, fillWith = ' ', fontWidth = 1) {
    ...
    // 需要填充的字符数量
    let fillCount =
      lineWidth - ((utils.getStringWidth(text1) + utils.getStringWidth(text2)) % lineWidth)
  ...
  printLCR(text1, text2, text3, fillWith = ' ', fontWidth = 1) {
    ...
    let fillCount =
      lineWidth -
      ((utils.getStringWidth(text1) + utils.getStringWidth(text2) + utils.getStringWidth(text3)) %
        lineWidth)
